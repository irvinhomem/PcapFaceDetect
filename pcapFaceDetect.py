...
def http_assembler(PCAP):
    ...
    for session in sessions:
        ...
        for packet in sessions[session]:
            try:
                if packet.dport == 80 or packet.sport == 80: #packet[TCP].dport packet[TCP].sport
            ...
            #Extract the raw image and return the image type and the binary body of the image itself
...
def face_detect(path, file_name):
    img = cv2.imread(path)
    cascade = cv2.CascadeClassifier('~/opencv/data/haarcascades/haarcascade_upperbody.xml')
